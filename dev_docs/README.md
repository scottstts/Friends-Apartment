...
| [audio.md](audio.md) | The generated theme loop: MIDI-derived score, parity proof, synth bank, mix, transport |
...
src/
  ...
  audio/             the generated theme loop (no audio files anywhere)
    synth.ts           instrument bank, per-part pitch buses (bend/vibrato), FX
    score.ts           GENERATED by tools/midi-to-score.mjs — notes, bends, mods, mix
    music.ts           track-to-voice casting, lookahead scheduler, scene-scoped state
  ui/
    ui.ts              procedural hallway landing, pause, loading, fatal veils
    sound.ts           pause-veil sound row: master-volume slider + brass mute speaker
  scenes/
...
tools/               authoring-time only, never bundled
  midi-to-score.mjs    assets/*.mid → src/audio/score.ts   (npm run score)
  verify-parity.mjs    proves score.ts *is* the MIDI       (npm run verify:score)
```
