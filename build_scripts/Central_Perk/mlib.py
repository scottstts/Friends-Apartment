...
def densify(pts, d=0.05):
    """Insert a point d before and d after every interior corner.

    tube_along puts one ring per path point and lofts between them, so a ring
    skewed onto a corner's mitre bisector drags that skew across the whole of
    the adjoining straight - a 4 m skirting run built from three points is a
    twisted surface, not a moulding lying on a wall.  Pinning the direction
    just short of each corner confines the mitre to the corner."""
    out = [tuple(pts[0])]
    for i in range(1, len(pts) - 1):
        ...
        if li > 2.5 * d:
            out.append(tuple(b - din / li * d))
        out.append(tuple(b))
        if lo > 2.5 * d:
            out.append(tuple(b + dout / lo * d))
    out.append(tuple(pts[-1]))
    return out


def tube_along(name, path, profile, cname=None, close_path=False,
               up=(0, 0, 1), cap=True, miter=False):
    """Sweep a closed 2D profile (list of (a,b)) along a 3D path.

    With miter=True the lateral half of the profile is widened by 1/cos of the
    half-turn at each corner, so a moulding keeps its true section round a
    corner instead of being pinched to cos(45) of it."""
    ...
    for i, p in enumerate(P):
        ...
        k = 1.0
        if miter and 0 < i < n - 1:
            e = (P[i] - P[i - 1])
            if e.length > 1e-9:
                k = 1.0 / max(0.4, e.normalized().dot(t))
        rings.append([tuple(p + s * (a * k) + u2 * b) for (a, b) in profile])
...
# ============================================================ central_perk add-ons

def wall_run(name, p0, p1, t, z0, z1, holes=(), cname=None, side=+1,
             cap0=True, cap1=True):
    """A straight wall between two plan points, built as ONE welded solid.

    `t` is measured from the p0->p1 line towards `side` (+1 = left of the
    direction of travel), so the *inner* face of every wall lands exactly on
    its layout line and nothing has to be nudged afterwards.
    `holes` are (u0, u1, hz0, hz1) in metres along the run - doors and windows
    are cut as part of the vertex grid, not booleaned, so the result stays
    quad-dominant and manifold.
    """
    A, B = Vector((p0[0], p0[1], 0.0)), Vector((p1[0], p1[1], 0.0))
    d = (B - A); L = d.length; d = d / L
    n = Vector((-d.y, d.x, 0.0)) * (t * side)

    # Snap before de-duplicating.  Two hole edges derived by different routes
    # land on the same value to within float noise, and an un-snapped set()
    # keeps both - which builds a sliver column a few atoms wide, and with it
    # a row of zero-area faces and duplicate vertices.
    q = lambda v: round(v, 5)
    holes = [tuple(q(x) for x in hh) for hh in holes]
    us = sorted(set([q(0.0), q(L)] + [v for hh in holes for v in (hh[0], hh[1])]))
    zs = sorted(set([q(z0), q(z1)] + [v for hh in holes for v in (hh[2], hh[3])]))
    nu, nz = len(us) - 1, len(zs) - 1

    def solid(i, j):
        if i < 0 or j < 0 or i >= nu or j >= nz:
            return False
        u0, u1, c0, c1 = us[i], us[i + 1], zs[j], zs[j + 1]
        for (a, b, e, f) in holes:
            if a - 1e-6 <= u0 and u1 <= b + 1e-6 and e - 1e-6 <= c0 and c1 <= f + 1e-6:
                return False
        return True
    ...
    def vid(i, j, s):
        ...
        if k not in idx:
            idx[k] = len(verts)
            p = A + d * us[i] + (n if s else Vector((0, 0, 0)))
            verts.append((p.x, p.y, zs[j]))
        return idx[k]
    ...
    for i in range(nu):
        for j in range(nz):
            if not solid(i, j):
                continue
            a, b = vid(i, j, 0), vid(i + 1, j, 0)
            c, e = vid(i + 1, j + 1, 0), vid(i, j + 1, 0)
            faces.append((a, b, c, e))
            a2, b2 = vid(i, j, 1), vid(i + 1, j, 1)
            c2, e2 = vid(i + 1, j + 1, 1), vid(i, j + 1, 1)
            faces.append((e2, c2, b2, a2))
            if not solid(i, j - 1):
                faces.append((b, a, a2, b2))
            if not solid(i, j + 1):
                faces.append((e, c, c2, e2))
            if (not solid(i - 1, j)) and (i > 0 or cap0):
                faces.append((a, e, e2, a2))
            if (not solid(i + 1, j)) and (i < nu - 1 or cap1):
                faces.append((c, b, b2, c2))
    ob = mesh_obj(name, verts, faces, cname)
    recalc_normals(ob)
    return ob
...
def slab(name, poly, z0, z1, cname=None):
    """Extrude a plan polygon (CCW) into a floor / ceiling slab."""
    return prism(name, poly, z0, z1, cname)
...
def poly_offset(poly, d):
    ...
    out = []
    for i in range(n):
        a = Vector(poly[i - 1] + (0,)) if len(poly[i - 1]) == 2 else Vector(poly[i - 1])
        b = Vector(poly[i] + (0,)) if len(poly[i]) == 2 else Vector(poly[i])
        c = Vector(poly[(i + 1) % n] + (0,)) if len(poly[(i + 1) % n]) == 2 else Vector(poly[(i + 1) % n])
        e0 = (b - a); e0.z = 0; e0.normalize()
        e1 = (c - b); e1.z = 0; e1.normalize()
...
def grid_plane(name, poly, z, nx, ny, cname=None, flip=False):
    """A floor plane subdivided into a grid (so it can take vertex-level
    shading and does not read as one giant quad), clipped to a rectangle."""
    xs = [p[0] for p in poly]; ys = [p[1] for p in poly]
    x0, x1, y0, y1 = min(xs), max(xs), min(ys), max(ys)
    verts, faces = [], []
    for j in range(ny + 1):
        for i in range(nx + 1):
            verts.append((lerp(x0, x1, i / nx), lerp(y0, y1, j / ny), z))
    for j in range(ny):
        for i in range(nx):
            a = j * (nx + 1) + i
...
def chamfer_pts(a, b, r):
    """Two points that replace corner b with a chamfer of leg r (2D)."""
    A, B = Vector(a), Vector(b)
    d = (A - B).normalized()
    return (B + d * r)


def clean_mesh(obj, dist=2e-5, weld=True):
    """Dissolve zero-area faces and weld coincident vertices.

    Lofts and joins leave both behind wherever a ring collapses - the fold at
    the top of a bag, the pole of a lathe, a mitre that closes on itself.
    They are invisible until something asks for a normal there, at which
    point shading breaks along a whole edge loop.  Cheap to remove, so the
    build removes them everywhere rather than hunting them one at a time."""
    me = obj.data
    bm = bmesh.new()
    bm.from_mesh(me)
    if weld:
...
# Text is tessellated by the font engine, not by this build.  Its coincident
# points are where two contours meet and welding them is not ours to do.
NOWELD = ("Menu_", "Neon_", "Service_t")


def clean_all(dist=2e-5, skip=()):
    n = 0
    for o in bpy.data.objects:
        if o.type != 'MESH' or o.name in skip:
            continue
        before = (len(o.data.polygons), len(o.data.vertices))
        clean_mesh(o, dist, weld=not o.name.startswith(NOWELD))
        if (len(o.data.polygons), len(o.data.vertices)) != before:
            n += 1
    return n
